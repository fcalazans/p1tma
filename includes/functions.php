...
function validateHeader($modCode, $modTitle, $modTutor, $modDate)
{
    ...
    if (trim($modCode) == '') {
        ...
        $headerErrorCheck++;
    } elseif (!in_array(substr($modCode, -2), ['T1', 'T2', 'T3'])
        && (!in_array(substr($modCode, 2), ['PP', 'P1', 'DT']))
        || !in_array(substr($modCode, 2, -2), ['1617', '1718', '1819', '1920'])) {
...
function studentData($arrID, $arrMark)
{
    ...
    for ($i = 0; $i < count($arrID); $i++) {
        if (strlen($arrID[$i]) != 8
            || !is_numeric($arrID[$i])
            || !is_numeric($arrMark[$i])
            || $arrMark[$i] > 100
            || $arrMark[$i] < 0) {
            echo $arrID[$i]
            . ' : '
            . $arrMark[$i]
            . ' : ERROR NOT TO BE INCLUDED</br>';
        } else {
...
/**
 * Function analyze the correct data from students and marks to display only the valid information.
 *
 * @param [array] $arrID
 * @param [array] $arrMark
 * @return void
...
function analytics($arrID, $arrMark)
{
    ...
    for ($i = 0; $i < count($arrID); $i++) {
        if ((strlen($arrID[$i]) == 8)
            ...
            if ($arrMark[$i] > 70) {
                $distinction++;
            } elseif ($arrMark[$i] >= 60 and $arrMark[$i] < 70) {
                $merit++;
            } elseif ($arrMark[$i] >= 40 and $arrMark[$i] < 60) {
                $pass++;
            } else {
                $fail++;
            }
...
function mmmr($array, $output = 'mean')
{
    ...
    if (!is_array($array)) {
        ...
        switch ($output) { //determine statistical output required
            ...
            case 'mode': //most frequent value in a list; N.B. will only find a unique mode or no mode;
                ...
                foreach ($v as $k => $v) {
                    /*
                    determine if a unique most frequent number,
                    or return NULL by only looking at first two keys and frequency
